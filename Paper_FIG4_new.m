...
f           = figure('Units', 'normalized', 'Position', [0 0 .6 1]); axis off
set(f,'color', [1 1 1]);
...
for iAn = 1:2
    ...
    ant8 = []; pos8 = []; ant12=[]; pos12 = []; aIdx = []; pIdx = [];    
    ant8        = z8(:,:,idx & ~post);
    pos8        = z8(:,:,idx & post);
    ant12       = z12(:,:,idx & ~post);
    pos12       = z12(:,:,idx & post);
    
    for m = 1:size(ant12,3)
        aIdx(m) = isnan(nanmean(nanmean(ant12(:,:,m))));
    end
    
    for m = 1:size(pos12,3)
        pIdx(m) = isnan(nanmean(nanmean(pos12(:,:,m))));
    end
...
%         ax.YTickLabel = [fr(1) fr(25) fr(50) fr(75) fr(100)];
        ...
        if iPlot == 1
            text(10,95, ['n = ' num2str(size(ant8,3))])
        elseif iPlot == 2
            text(10,95, ['n = ' num2str(size(pos8,3))])
        end
...
for iAn = 1:2
    ...
    for iCond = 1:2
        if iAn == 1 && iCond == 1
            ax = axes('Position',[.06 rowB(1) dimB]); hold on
            str = 'ANT';
        elseif iAn == 1 && iCond == 2
            ax = axes('Position',[.06 rowB(2) dimB]); hold on
            str = 'POS';
        elseif iAn == 2 && iCond == 1
            ax = axes('Position',[.58 rowB(1) dimB]); hold on
            str = 'ANT';
        else
            ax = axes('Position',[.58 rowB(2) dimB]); hold on
            str = 'POS';
        end
...
text(clm(1)+.05,.45, 'Alpha/Beta', 'Parent', ax0, 'FontSize', 20, 'Color', 'k', 'FontWeight', 'bold')
text(clm(1)+.25,.45, 'Gamma', 'Parent', ax0, 'FontSize', 20,'Color', 'k', 'FontWeight', 'bold')
text(clm(3)+.05,.45, 'Alpha/Beta', 'Parent', ax0, 'FontSize', 20, 'Color', 'k', 'FontWeight', 'bold')
text(clm(3)+.24,.45, 'Gamma', 'Parent', ax0, 'FontSize', 20,'Color', 'k', 'FontWeight', 'bold')
...
text(0,row(4)+.11, 'a', 'Parent', ax0, 'FontSize', 30, 'Color', 'k', 'FontWeight', 'bold')
text(0,row(2)+.11, 'b', 'Parent', ax0, 'FontSize', 30, 'Color', 'k', 'FontWeight', 'bold')  
text(0,rowB(1)+.21, 'c', 'Parent', ax0, 'FontSize', 30, 'Color', 'k', 'FontWeight', 'bold')  
